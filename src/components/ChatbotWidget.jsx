...
const ChatBotWidget = () => {
  ...
  const handleSendText = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:5000/api/ChatBot?message=${encodeURIComponent(userMessage)}`, {
        method: "GET",
        headers: {
  ...
  const toggleListening = () => {
    ...
    recognition.onresult = async (event) => {
      ...
      try {
        ...
        const response = await fetch("http://127.0.0.1:5000/api/ChatBot/voice", {
          method: "POST",
          headers: { 
