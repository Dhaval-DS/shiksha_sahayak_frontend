...
import { useLanguage } from '../context/LanguageContext'; // 🚀 IMPORTED LANGUAGE HOOK
...
const TeacherProfile = () => {
  ...
  const handleProfileUpdate = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch("http://127.0.0.1:5000/api/teacher/profile", {
        method: "PUT",
        headers: { 
  ...
  const handlePasswordUpdate = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch("http://127.0.0.1:5000/api/teacher/change-password", {
        method: "PUT",
        headers: { 
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}` 
