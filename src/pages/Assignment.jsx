...
const Assignment = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const classRes = await fetch("http://127.0.0.1:5000/api/classes/getAllClasses", {
        ...
        const assignRes = await fetch("http://127.0.0.1:5000/api/assignments/getAllAssignments", {
          headers: { "Authorization": `Bearer ${token}` }
        });
  ...
  const fetchAssignmentsOnly = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch("http://127.0.0.1:5000/api/assignments/getAllAssignments", {
        headers: { "Authorization": `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const url = modalMode === 'add' 
        ? "http://127.0.0.1:5000/api/assignments/create" 
        : `http://127.0.0.1:5000/api/assignments/${editingAssignmentId}`;
  ...
  const handleViewDetails = async (assignment) => {
    ...
    try {
      ...
      const studentRes = await fetch(`http://127.0.0.1:5000/api/students/getAllStudents?classId=${assignment.classId}`, {
        headers: { "Authorization": `Bearer ${token}` }
      });
      const studentsData = await studentRes.ok ? await studentRes.json() : [];
      setClassStudents(studentsData);

      const submissionRes = await fetch(`http://127.0.0.1:5000/api/submissions/assignment/${assignment.id}`, {
        headers: { "Authorization": `Bearer ${token}` }
      });
  ...
  const handleGradeSubmit = async () => {
    ...
    try {
      ...
      for (let studentId of draftKeys) {
        ...
        await fetch("http://127.0.0.1:5000/api/submissions/submit", {
          method: "POST",
                              ...
                              <div className="flex items-center space-x-3">
                                ...
                                {student.photo ? (
                                  <img 
                                    ...
                                    src={`http://127.0.0.1:5000/api/students/StudentPhoto/${student.id}`} 
