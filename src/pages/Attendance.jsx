...
const AttendancePage = () => {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      try {
        ...
        const res = await fetch("http://127.0.0.1:5000/api/classes/getAllClasses", {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const rosterRes = await fetch(`http://127.0.0.1:5000/api/students/getAllStudents?classId=${selectedClassId}`, { headers });
      const rosterData = rosterRes.ok ? await rosterRes.json() : [];

      const attRes = await fetch(`http://127.0.0.1:5000/api/attendance/class/${selectedClassId}/date/${formattedDate}`, { headers });
  ...
  useEffect(() => {
    if (showHistory && selectedClassId) {
      const fetchHistory = async () => {
        const token = localStorage.getItem("token");
        try {
          const res = await fetch(`http://127.0.0.1:5000/api/attendance/history/${selectedClassId}`, {
            headers: { "Authorization": `Bearer ${token}` }
          });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("http://127.0.0.1:5000/api/attendance/submit", {
        method: "POST",
        headers: {
                  ...
                  {filteredStudents.map((student) => (
                    <div key={student.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border border-slate-100 bg-white gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        {student.photo ? (
                          <img
                            src={`http://127.0.0.1:5000/api/students/StudentPhoto/${student.id}?t=${new Date().getTime()}`}
