...
const RebuildMemoryButton = ({ classId }) => {
  ...
  const handleRebuild = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:5000/api/attendance/rebuild-encodings/${classId}`, {
        method: "POST",
...
const SmartAttendance = () => {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch("http://127.0.0.1:5000/api/classes/getAllClasses", {
          headers: { "Authorization": `Bearer ${token}` }
        });
        if (response.ok) {
          const data = await response.json();
  ...
  useEffect(() => {
    ...
    const fetchStudents = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(`http://127.0.0.1:5000/api/students/getAllStudents?classId=${selectedClassId}`, {
          headers: { "Authorization": `Bearer ${token}` }
        });
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch("http://127.0.0.1:5000/api/attendance/process-photo", {
        method: "POST",
        headers: { "Authorization": `Bearer ${token}` },
        body: formData,
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch("http://127.0.0.1:5000/api/attendance/submit", {
        method: "POST",
        headers: {
