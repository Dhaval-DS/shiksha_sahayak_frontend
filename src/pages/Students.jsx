...
const Students = () => {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      try {
        ...
        const response = await fetch("http://127.0.0.1:5000/api/classes/dropdown", {
  ...
  const fetchStudents = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch("http://127.0.0.1:5000/api/students/getAllStudents", {
        headers: { "Authorization": `Bearer ${token}` }
      });
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const url = modalMode === 'add' 
        ? "http://127.0.0.1:5000/api/students/createStudent" 
        : `http://127.0.0.1:5000/api/students/updateStudent/${editingStudentId}`;
  ...
  const handleTrainAI = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch("http://127.0.0.1:5000/api/attendance/train-student", {
        method: "POST",
        headers: { "Authorization": `Bearer ${token}` },
        body: formData
      });
  ...
  const AvatarIcon = ({ student, size = "md" }) => {
    ...
    if (student.photo) {
      ...
      return <img src={`http://127.0.0.1:5000/api/students/StudentPhoto/${student.id}?t=${imageHash}`} alt={student.firstName} className={`${dimensions} rounded-full object-cover border-4 border-white shadow-md`} />;
    }
    return (
